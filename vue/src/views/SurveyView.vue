...
function deleteQuestion(question: any) {
    model.value.questions = model.value.questions.filter((q: any) => q.id !== question.id )
}

function questionChange(question: any) {
    const questions = model.value.questions.map((q: any) => {
        if (q.id === question.id) {
    ...
    Object.assign(model.value.questions, questions)
}
